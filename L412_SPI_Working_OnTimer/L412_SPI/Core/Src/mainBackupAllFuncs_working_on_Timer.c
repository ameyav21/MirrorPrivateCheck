...
//   const multisample_t* pADXL355oversampled; // Disallow changes to contents. Intentionally uninitialized.
//   uint16_t DataReadyWaitLimit_ms;     // Intentionally uninitialized.
//   uint16_t ADXL355samplesInPeriod;    // Intentionally uninitialized.
//
...
//
//   DataReadyWaitLimit_ms = (uint16_t) ceil((2.0 * 1000.0 / ADXLsampling[OptimizedSPS].DataRate)); // Double expected period.
//   ADXL355samplesInPeriod = (uint16_t) (ADXLsampling[OptimizedSPS].DataRate * ADXLsamplePeriod);
  ...
  while (1)
  {
	...
	// Get current time (microseconds)
	 timer_val = __HAL_TIM_GET_COUNTER(&htim16);
	  HAL_Delay(5);
	// Get time elapsed
	timer_val2 = __HAL_TIM_GET_COUNTER(&htim16) - timer_val;
	...
	if (timer_val>=5){
		uart_buf_len = sprintf(uart_buf, "Timer above 5\r\n");
		HAL_UART_Transmit(&huart2, (uint8_t *)uart_buf, uart_buf_len, 100);
	}

	  int checknum = check_adxl(0x00, ANALOG_DEVICES_ID);
	  if (checknum==1){
		  char *mex="True............";
		  char line[2] = "\r\n";
		  HAL_UART_Transmit(&huart2, (uint8_t *) mex, 15, 10);
	  ...
	  ADXL355DataReady();
	  HAL_Delay(200);
	  //pADXL355oversampled = OverSampleADXL355(ADXL355samplesInPeriod,DataReadyWaitLimit_ms);
	  HAL_Delay(200);
...
//      if (status==1) {
//    	  char *mex="AccelerometeRT..";
//    	      char line[2] = "\r\n";
//    	      HAL_UART_Transmit(&huart2, (uint8_t *) mex, 15, 10);
//    	      HAL_UART_Transmit(&huart2, (uint8_t *) line, 2, 10);
//      }
//      else{
//    	      	  char *mex="AccelerometeRF..";
//    	      	      char line[2] = "\r\n";
//    	      	      HAL_UART_Transmit(&huart2, (uint8_t *) mex, 15, 10);
//    	      	      HAL_UART_Transmit(&huart2, (uint8_t *) line, 2, 10);
//      }

//      int32_t AxisData = ReadADXL355acceleration((eAxis_t) aAxis);
//
//      int32_t x=AxisData;
//      int digs[12], count=0;
...
//          msg[i++] = digs[count] +'0';
//      }
//      HAL_UART_Transmit(&huart2, (uint8_t *) msg, 15, 10);
//      HAL_UART_Transmit(&huart2, (uint8_t *) line, 2, 10);
